class Snake
{
	...
	think()
	{
		...
		//should you speed up boi
		var input9 = this.moves/250;
		
		//distance to tail
		var upDist=canvas.height,downDist=canvas.height,leftDist=canvas.width,rightDist= canvas.width;
		for(var i in this.body)
		{
		...
		if(i==0 || i==1 || i==2)continue;
			...
			if(x==this.body[0].x)
			{
				if(y<this.body[0].y)
				{
					if(this.body[0].y - y<upDist)upDist=this.body[0].y - y;
				}
				if(y>this.body[0].y)
				{
					if(y - this.body[0].y<downDist)downDist=y - this.body[0].y;
				}
			}
			if(y==this.body[0].y)
			{
				if(x<this.body[0].x)
				{
					if(this.body[0].x - x<leftDist)leftDist=this.body[0].x - x;
				}
				if(x>this.body[0].x)
				{
					if(x - this.body[0].x<rightDist)rightDist=x - this.body[0].x;
				}
			}
		}
		var input10 = upDist/canvas.height;
		var input11 = downDist/canvas.height;
		var input12 = leftDist/canvas.width;
		var input13 = rightDist/canvas.width;
		...
		inputArray.push(input10);
		inputArray.push(input11);
		inputArray.push(input12);
		inputArray.push(input13);
	...
	calculateFitness()
	{
		...
		// this.fitness = (this.traveledDistance*0.75)/10
		// this.fitness-= (distance/45)/10;
		// this.fitness += (Math.pow(this.points,4))/10;
		// this.fitness = Math.pow(this.fitness,2);
		
		if (this.points < 10) 
		{
		  this.fitness = Math.floor(this.traveledDistance *this.traveledDistance * Math.pow(2, (Math.floor(this.points))));
		 
		} else {
		  this.fitness = this.traveledDistance*this.traveledDistance;
		  this.fitness *= Math.pow(2, 10);
		  this.fitness *=(this.points-9);
		}
